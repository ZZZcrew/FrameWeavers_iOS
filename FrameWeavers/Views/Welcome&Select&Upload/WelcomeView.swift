        ...
        GeometryReader { geometry in
            ScrollView {
                VStack(spacing: 0) {
                    ...
                    // 图标与文字间距
                    Spacer()
                        .frame(height: max(geometry.size.height * 0.05, 30))
                    ...
                    // 底部弹性空间 - 减少空隙
                    Spacer()
                        .frame(minHeight: geometry.size.height * 0.02)
                }
                .frame(maxWidth: .infinity) // 确保内容水平居中
                .padding(.vertical, 20) // 添加上下内边距
            }
            .frame(maxWidth: .infinity) // 确保 ScrollView 占满宽度
        }
